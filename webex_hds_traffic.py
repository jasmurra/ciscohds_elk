...
base = 'https://api.webex.com/v1'

# who am I?
r = requests.get('https://api.webex.com/v1/people/me', headers=headers)
r.raise_for_status()
me_id = r.json()['id']

# get all people
r = requests.get('https://api.webex.com/v1/people', params={'max':1000}, headers=headers)
r.raise_for_status()
...
while True:
    ...
    r = requests.post('https://api.webex.com/v1/rooms', headers=headers, data=json.dumps({'title' : 'zz auto generated {}'.format(str(uuid.uuid4()))}))
    r.raise_for_status()
    space_id = r.json()['id']
    ...
    r = requests.post('https://api.webex.com/v1/messages', headers=headers, data=json.dumps({'roomId' : space_id, 'text' : 'Space automatically created to cause some traffic'}))
